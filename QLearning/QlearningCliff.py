...
class Environment:
    ...
    def step(self, action):
        ...
        done = (state_next[0] == 3) and ((state_next[1] == 9) or state_next[1] != 0)
        # Update state
    ...
    def _build_rewards(self):
        ...
        r_cliff = -100 # penalty for falling off cliff
        R = r_nongoal * np.ones(self.state_dim + self.action_dim, dtype=float)  # R[s,a]
        R[2, 9, self.action_dict["down"]] = r_goal  # arrive from above
        R[3, 8, self.action_dict["right"]] = r_goal
        R[2, 1, self.action_dict["down"]] = r_cliff
        R[2, 2, self.action_dict["down"]] = r_cliff
        R[2, 3, self.action_dict["down"]] = r_cliff
        R[2, 4, self.action_dict["down"]] = r_cliff
        R[2, 5, self.action_dict["down"]] = r_cliff
        R[2, 6, self.action_dict["down"]] = r_cliff
        R[2, 7, self.action_dict["down"]] = r_cliff
        R[2, 8, self.action_dict["down"]] = r_cliff
        R[3, 0, self.action_dict["right"]] = r_cliff
...
class Agent:
    ...
    def display_greedy_policy(self):
        ...
        greedy_policy = -np.ones((self.state_dim[0], self.state_dim[1]), dtype=int)
